...
async def hyperion(request: Request):
    ...
    ticker_map = await _load_ticker_map_for_types(["SPOT", "FUTURES", "SWAP"])
    ...
    if root_kind == "instrument":
        ...
        raw = _INSTR_CACHE.get(symbol) if symbol else None
        if raw is None:
            return JSONResponse(
                {
                    "data": {root_kind: None},
...
async def md_quotes(broker_symbol: str):
    ...
    payload = {
        ...
        "exchange": exchange_out,
        "description": None,
        "prev_close_price": open_price, #используем open24h
