...
// DEV MODE: run Astras UI without Alor SSO (no redirects). Set to false to restore SSO.
const ASTRAS_DEV_NO_SSO = true;
// A minimal unsigned JWT (alg=none) with required fields for the UI (sub, clientid, portfolios, exp).
const ASTRAS_DEV_JWT = 'eyJhbGciOiJub25lIiwidHlwIjoiSldUIn0.eyJzdWIiOiJERVYiLCJjbGllbnRpZCI6IkRFViIsInBvcnRmb2xpb3MiOiIiLCJleHAiOjQxMDI0NDQ4MDB9.';
...
export class ClientAuthContextService implements UserContext, SessionContext, OnDestroy {
  ...
  setRefreshToken(refreshToken: string): void {
    ...
    private initDevSession(): void {
    // Create a minimal in-memory auth state so the UI can work without SSO.
    const now = Date.now();
    const expirationTime = now + 1000 * 60 * 60 * 24 * 365; // 1 year
    const jwt = ASTRAS_DEV_JWT;
    const refreshToken = 'DEV';

    this.apiTokenProviderService.clearToken();
    this.apiTokenProviderService.updateTokenState({
      token: jwt,
      expirationTime,
      refreshCallback: () => this.initDevSession()
    });
